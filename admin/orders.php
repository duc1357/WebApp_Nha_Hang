                ...
                .then(data => {
                    ...
                    data.orders.forEach(o => {
                        ...
                        tr.innerHTML = `
                            ...
                            <td>
                                <div><strong style="font-size:15px; color:#e67e22;">${Number(o.final_total || o.total_amount).toLocaleString('vi-VN')} đ</strong></div>
                                ${o.discount_amount > 0 ? `<div style="text-decoration:line-through; color:#999; font-size:12px;">${Number(o.total_amount).toLocaleString('vi-VN')} đ</div>` : ''}
                                ${o.discount_amount > 0 ? `<div style="color:green; font-size:11px;">- ${Number(o.discount_amount).toLocaleString('vi-VN')}</div>` : ''}
                            </td>
                            <td>
                                ${o.table_id ? 
                                    `<span class="badge" style="background:#e3f2fd; color:#0d47a1;">Tại bàn: ${o.table_id}</span>` : 
                                    (o.address ? `<div style="font-size:13px;">Giao: <b>${o.address}</b></div>` : '<span style="color:#999; font-style:italic;">Mua lại quầy</span>')
                                }
                                ${o.note ? `<div style="margin-top:4px; font-size:12px; color:#c0392b; font-style:italic;">" ${o.note} "</div>` : ''}
                            </td>
                            <td><span class="badge ${statusClass}">${statusText}</span></td>
                            <td style="text-align:center;">
                                ${o.status !== 'paid' && o.status !== 'cancelled' ? 
                                    `<button onclick="updateStatus(${o.id}, 'paid')" class="btn-action btn-approve" title="Duyệt đơn">
                                     ...
                                     <button onclick="updateStatus(${o.id}, 'cancelled')" class="btn-action btn-cancel" title="Duyệt hủy">
